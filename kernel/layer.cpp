...
Layer *LayerManager::FindLayerByPosition(Vector2D<int> position, unsigned int exclude_id) const
{
    auto it = std::find_if(layer_stack_.rbegin(), layer_stack_.rend(), [position, exclude_id](Layer *layer)
                           {
                               ...
                               return win_pos.x <= position.x && position.x<win_end_pos.x &&win_pos.y<=position.y && position.y < win_end_pos.y; });
...
{
    auto it = std::find_if(layers_.begin(), layers_.end(), [id](const std::unique_ptr<Layer> &elem)
                           { return elem->ID() == id; });
    if (it == layers_.end())
    {
        return nullptr;
