...
void SwitchEhci2Xhci(const pci::Device &xhc_dev)
{
    ...
    {
        if (pci::devices[i].class_code.Match(0x0cu, 0x03u, 0x20u) && pci::ReadVendorId(pci::devices[i]) == 0x8086)
        {
...
extern "C" void KernelMainNewStack(const FrameBufferConfig &frame_buffer_config_ref, const MemoryMap &memory_map_ref)
{
    ...
    for (int i = 0; i < pci::num_device; i++)
    {
        const auto &d = pci::devices[i];
        auto vendor_id = pci::ReadVendorId(d);
        auto class_code = pci::ReadClassCode(d.bus, d.device, d.function);

        Log(kDebug, "%d.%d.%d: vendor %04x, class %08x, header_type %02x\n",
            d.bus, d.device, d.function, vendor_id, class_code, d.header_type);
    }
